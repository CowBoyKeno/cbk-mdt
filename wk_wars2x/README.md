# Shout out to the original creator/author WolfKnight98, Thank you for all your work!
...
Current custom build version: 1.3.2-cbk1
...
- 03-11-2026 (CowBoyKeno) - v1.3.2-cbk1
	- Added direct `cbk-mdt` event integration for radar lock/unlock ingest.
	- Synced radar and plate-reader lock behavior per antenna to prevent mismatched ALPR entries.
	- Added support for MDT-triggered force unlock from ALPR close action.
	- Added client token handling for secured MDT radar ingest/unlock flow.

## CowBoyKeno Custom Integration (cbk-mdt)

The following project-specific changes were added by CowBoyKeno for tight MDT integration:

- Radar lock events are sent directly to `cbk-mdt` using `cbk_mdt:server:radarHit`.
- Radar unlock events are sent directly to `cbk-mdt` using `cbk_mdt:server:radarUnlock`.
- Plate-reader locks include MDT radar token context when forwarding `wk:onPlateLocked`.
- Radar and plate-reader lock behavior is synchronized by antenna (front/rear).
- Closing Front/Rear ALPR entries in MDT can force-unlock the matching antenna on the client.
- Radar target identity is reused for plate locking, reducing mismatched speed/plate pairs.

### Security Notes for This Integration

- MDT now issues short-lived client radar tokens.
- Wraith passes tokenized payloads for radar hit/unlock-related ingestion.
- Server-side source checks and token checks are required before ALPR ingest/unlock operations are accepted.
